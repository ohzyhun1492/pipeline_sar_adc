%% Pipeline SAR ADC (Multi-CDAC: Big/Small Split) Automation & Monte Carlo Analysis
%  Fixed CDAC clock skew, multiple realizations
clc; clear; close all;

%% 1. 글로벌 및 시뮬레이션 반복 설정
num_simulations = 100;                        % 반복할 시뮬레이션 횟수
enob_history = zeros(num_simulations, 1);
sndr_history = zeros(num_simulations, 1);
...
sigma_u_1st    = 0.0018;       % 1st Small CDAC (판정용) mismatch
sigma_u_1st_v2 = 0.0009;       % 1st Big   CDAC (residue) mismatch
sigma_u_2nd    = 0.005;        % 2nd Stage CDAC mismatch
...
fprintf('====================================================================\n');
fprintf('  ▶ CDAC Clock Skew (Big vs Small)      : %.2f ps\n', CDAC_skew * 1e12);
fprintf('--------------------------------------------------------------------\n');
...
fprintf(' 평균 SNDR: %.2f dB (표준편차: %.3f)\n', mean(sndr_history), std(sndr_history));
fprintf('====================================================================\n');
...
sgtitle(sprintf('Pipeline SAR ADC MC Statistics (skew = %.2f ps)', CDAC_skew*1e12), ...
